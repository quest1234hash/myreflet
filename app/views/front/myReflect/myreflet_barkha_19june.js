              ...
              db.query("SELECT *,tbl_myreflectid_doc_rels.createdAt as doc_create_date FROM tbl_myreflectid_doc_rels INNER JOIN tbl_documents_masters on tbl_myreflectid_doc_rels.doc_id=tbl_documents_masters.doc_id where tbl_myreflectid_doc_rels.deleted='0' and tbl_myreflectid_doc_rels.reflect_id="+reflect_id,{ type:db.QueryTypes.SELECT}).then(async function(document){
                  ...
                  if(document==""){
                    ...
                    if(country_id!=null){
                         ...
                         CountryModel.findOne({where:{country_id:country_id}}).then( async country =>{
                            ...
                            console.log("certified 1",certified);
                             async function forsend(){
                                res.render('front/myReflect/my-reflet-id-view',{session:req.session,result,additional_info_array,user,country:country.country_name,moment,allCountries,documents,allDocs,certified,didRequestOrNot});
                             }
                        ...
                        res.render('front/myReflect/my-reflet-id-view',{session:req.session,result,additional_info_array,user,moment,allCountries,country:null,documents,allDocs,certified,didRequestOrNot});
    ...
    form.parse(req, (err, fields, files) => {
        ...
        ipfs.files.add(testBuffer, function (err, file) {
            ...
            DocumentMasterModel.findAll({where:{deleted:"0",status:"active"}}).then(allDocs =>{
                DocumentReflectIdModel.create({doc_id:doc_id,doc_unique_code:id_number,reflect_id:reflect_id,issue_place,issue_date,expire_date:expiry_date}).then(doc =>{
...
exports.request_doc = async(req,res,next) =>{
    console.log("...........................................request_doc start*******....................................");
       ...
       // console.log(".....*******.....",doc_id);   
       await db.query("select * from tbl_admin_durations",{ type:db.QueryTypes.SELECT}).then(async function(due_date_data){
        ...
        var all_attested_images=await FilesDocModel.findAll({where:{user_doc_id:user_doc_id,type:"image",deleted:'0'}});
           ...
           if(all_attested_images.length>0)
           {
                for(j=0;j<all_attested_images.length;j++)
                {
                   ...
                   console.log('final_url : PPPPPPPPPPPPPPP : ',final_url)
                    await request(final_url,callback);
          ...
          var smtpTransport = nodemailer.createTransport({
           ...
           var html_data_part3=`<p>Please contact to administrator for more details.</p>
                 ...
                 console.log('attachments : PPPPPPPPPPPPPPP : ',attachments)
            ...
            smtpTransport.sendMail(mailOptions, function (err) {
                                console.log('MAIL SENTTTTTTTTTTTTT')
...
const get_digi_hash= async function(fun_hash,blob_url){
    var today=new Date();
    // console.log("hello-----------1 ");
    await Jimp.create(720,520,'#ffffff').then(async nova_new =>{
...
exports.Entity = (req,res,next )=> {
    ...
    //    MyReflectIdModel.findOne({where:{reflect_id:ref_id,deleted:"0"}})
       db.query('SELECT * FROM tbl_wallet_reflectid_rels INNER JOIN tbl_user_registrations ON tbl_wallet_reflectid_rels.reg_user_id=tbl_user_registrations.reg_user_id where tbl_wallet_reflectid_rels.deleted="0" AND tbl_wallet_reflectid_rels.reflect_id='+ref_id,{ type:db.QueryTypes.SELECT})
       .then(result => {
           DocumentReflectIdModel.findAll({where:{reflect_id:ref_id}}).then(doc_data=>{
 
             db.query("SELECT *,tbl_myreflectid_doc_rels.createdAt as doc_create_data FROM tbl_myreflectid_doc_rels INNER JOIN tbl_documents_masters on tbl_myreflectid_doc_rels.doc_id=tbl_documents_masters.doc_id where tbl_myreflectid_doc_rels.reflect_id="+ref_id,{ type:db.QueryTypes.SELECT}).then(async function(document){
                 var documents;
                //  if(document==""){
                   documents = null;
                //  }else{
            ...
            if(content.self_attested_hash){
             ...
             console.log("self attested 1---",all_self_attested);
                ...
                //   console.log("---------doc_content---------- ",documents);
                 ...
                 DocumentMasterModel.findAll({where:{deleted:"0",status:"active"}}).then(docMasterData=>{
                        ...
                        db.query('SELECT * FROM tbl_wallet_reflectid_rels INNER JOIN tbl_user_registrations ON tbl_wallet_reflectid_rels.reg_user_id=tbl_user_registrations.reg_user_id where tbl_wallet_reflectid_rels.deleted="0" AND tbl_wallet_reflectid_rels.reflect_id='+result.reflect_id,{ type:db.QueryTypes.SELECT})
                         .then(result => {
                             DocumentReflectIdModel.findAll({where:{reflect_id:result[0].reflect_id}}).then(doc_data=>{
                                 ...
                                 db.query("SELECT * FROM tbl_myreflectid_doc_rels INNER JOIN tbl_documents_masters on tbl_myreflectid_doc_rels.doc_id=tbl_documents_masters.doc_id where tbl_myreflectid_doc_rels.reflect_id="+result[0].reflect_id,{ type:db.QueryTypes.SELECT}).then(async function(document){
                                     var documents;
                                     if(document==""){
                                       documents = null;
                                     }else{
...
// //    console.log(".........req...",exp_date)
 ...
 await  ipfs.files.add(testBuffer,async function (err, file) {
    ...
    await DocumentReflectIdModel.create({doc_id:doc_name_id,doc_unique_code:doc_id_number,reflect_id:reflect_id,issue_place,issue_date,expire_date:exp_date}).then(async(doc) =>{
        // console.log("&&&&&&&&&&&&&&&&&&&&&&&&&",doc)
...
// }
async function  selfAttestedMail()
{
