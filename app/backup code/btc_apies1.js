...
exports.getRecentTenCryptoWallets=async function(req,res){
  ...
  try{
    ...
    //  console.log("detttttttttttttttttttt",userDet);   
       let allCypto =await CryptoWalletModel.findAll({where:{reg_user_id:user_id},limit:10,order: sequelize.literal('wallet_id DESC')})
       var respObj={
...
exports.getCryptoWallets=async function(req,res){
  let user_id=req.body.user_id;
  try{
       let cryptoWallets =await CryptoWalletModel.findAll({where:{reg_user_id:user_id}});
