...
exports.export_individual_document = async (req, res, next) => {
  ...
  if (certify == "0") {
    ...
    async function finalCall(type_val) {
      ...
      for ( i = 1; i <= type_val; i++) {
        zip.addLocalFile(
          `./app/uploads/document_pdf/${document_name}_${doc_unique_code}_${i}.pdf`
        );
        //console.log("zip path:",`./app/uploads/document_pdf/${document_name}_${doc_unique_code}_${i}.pdf`)
        try {
          fs.unlinkSync(`./app/uploads/document_pdf/${document_name}_${doc_unique_code}_${i}.pdf`)
          //file removed
        } catch(err) {
          console.error(err)
        }
      ...
      let path_zip = `./app/uploads/document_pdf/${downloadName}`;
      // save file zip in root directory
      //console.log("@@@-----", data);
      zip.writeZip(path_zip);
      //*Adm zip end */

     
      res
        .contentType("application/octet-stream")
        .send(`data:application/zip;base64,${data.toString("base64")}`);

        fs.unlink(path_zip, (err) => {
          if (err) {
            console.error(err)
            return
          }
