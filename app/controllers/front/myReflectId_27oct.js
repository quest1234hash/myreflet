                                      ...
                                      .then( async (userData) => {
                                          ...
                                          .then(async(notification) =>{
                                            ...
                                            console.log("notification done........");
                                                ...
                                                var k = 0
                                                await async.each(doc_id,async function (content, cb) {
                                                           ...
                                                           .then(async(success) =>{
                                                                ...
                                                                console.log("outside if final respones call");

                                                                 if(k==(doc_id.length-1)){
                    ...
                    // const fileUrl = new URL(`https://ipfs.io/ipfs/${hashes[j].hash}`);
					  ...
					  // const pdfDoc = await PDFDocument.load(pdf1)
                       ...
                       const imagePage = pdfDoc.insertPage(0);
                        ...
                        imagePage.drawText(name_text, { x:400, y: 100, size: 10 })

                        if(objOFNames.text_for_signature){
                            imagePage.drawText(objOFNames.text_for_signature, { x:400, y: 90, size: 10 })
                        }
      ...
      await db.query("SELECT * from tbl_wallet_reflectid_rels inner join tbl_user_wallets ON tbl_wallet_reflectid_rels.wallet_id=tbl_user_wallets.wallet_id inner join tbl_user_registrations on tbl_user_registrations.reg_user_id=tbl_user_wallets.reg_user_id WHERE tbl_wallet_reflectid_rels.reflectid_by!='digitalWallet' AND tbl_wallet_reflectid_rels.reg_user_id="+user_id+" and user_as='"+user_type+"'",{type:db.QueryTypes.SELECT}).then(async function(myreflectData){
              ...
              await  db.query("SELECT * FROM tbl_digital_wallet_rels INNER JOIN tbl_wallet_reflectid_rels ON tbl_digital_wallet_rels.dig_wal_reflect_id=tbl_wallet_reflectid_rels.reflect_id WHERE parent_reflect_id="+myreflectData[i].reflect_id+" AND tbl_digital_wallet_rels.deleted='0'",{type:db.QueryTypes.SELECT}).then(async function(digitalWalletData){
                    ...
                    myreflectData[i].digitalWallet = digitalWalletData
                         ...
                         for(var j=0;j< myreflectData.length;j++)
                         {
                            var reflect_id = myreflectData[j].reflect_id;

                            var user_as = myreflectData[j].user_as;
                                ...
                                // console.log('level_name : ',level_name)
                                 ...
                                 await   db.query("SELECT * FROM `tbl_allot_levels` inner join tbl_documents_masters on tbl_documents_masters.doc_id = tbl_allot_levels.doc_id WHERE level_id="+level_id+"  order by allot_level_id desc",{ type:db.QueryTypes.SELECT}).then( async(user_allot)=>{
                                                    ...
                                                    //    console.log('user_doc_id : ',doc_id)
                                                          ...
                                                          if(user_as =='client')
                                                          {
                                                              ...
                                                              await db.query("SELECT * FROM `tbl_request_documents` inner join tbl_myreflectid_doc_rels on tbl_request_documents.user_doc_id = tbl_myreflectid_doc_rels.user_doc_id INNER JOIN tbl_request_documents_files ON tbl_request_documents_files.request_doc_id =tbl_request_documents.request_doc_id WHERE doc_id = "+doc_id+" and  reflect_id = "+reflect_id+" and docfile_status = 'accept'",{ type:db.QueryTypes.SELECT}).then( async(user_doc)=>{
                                                                    ...
                                                                    // console.log('count_data : ',count_data)
                                                                    // console.log('level_length : ',level_length," level_length : ",level_name)
                                                                     if(count_data === level_length)
                                                                     {
                                                                          console.log('(user_doc.count_data : ',count_data,' level_length : ',level_length)
                  
                                              
                                                                                 obj_data[p] = {level_name,level_length}     
                                                                    
                                                                             console.log('inner : k ',p," i : ",i," k : ",obj_data[k])
                                                                             p++;
                                                                     }
                                    ...
                                    //  const data;
                                     ...
                                     let max = 0;
                                     var level_name;
                                            obj_data.forEach(obj_data => {
                                            if (obj_data.level_length > max) {
                                                max = obj_data.level_length;
                                                console.log("macvgghy : ",max)
                                                level_name = obj_data.level_name

                                            }
                                           
                                      });
                                    
                                         console.log("datadatadatadatadatadatadatadatadatadatadata :",max, level_name)

                                        myreflectData[j].level_name = level_name    
                                      }
                                      else
                                      {
                                        myreflectData[j].level_name = 'undefined'
