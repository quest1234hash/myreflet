...
exports.accept_request = async (req,res,next) =>{
  ...
  /*outer loop Start*/
    ...
    console.log("*****ACCCCCCCCCCCCCCCept****** ");

    var k=0;     
           async.each(request_doc_data,async function (content, cb) {
              
    console.log("*****request_doc_data****** ");
      ...
      // console.log("client_data----------********------ ",client_data);
          ...
          await db.query("SELECT * FROM tbl_wallet_reflectid_rels INNER JOIN tbl_user_registrations ON tbl_wallet_reflectid_rels.reg_user_id=tbl_user_registrations.reg_user_id WHERE tbl_wallet_reflectid_rels.deleted='0' AND tbl_wallet_reflectid_rels.reflect_id="+ver_reflect_id,{ type:db.QueryTypes.SELECT}).then(async function(verifier_data){
            ...
            var verifier_name;
              ...
              if(verifier_data.entity_company_name)
              {
                          verifier_name = verifier_data.entity_company_name;
              }
              else  if(verifier_data.rep_firstname)
              {
              	          verifier_name = verifier_data.rep_firstname;

              }
              else
              {
              	            verifier_name = verifier_data.full_name;
            ...
            // console.log("wallet_data----------********------ ",wallet_data);
                     ...
                     var jimp_doc;
                                         console.log("before await 1");
                                                       	console.log("detail-----2------ ",fun_hash,verifier_name,verifier_reflect_code);
              ...
              async function jimp_fun()
              {
              	...
              	console.log("detail----------- ",fun_hash,verifier_name,verifier_reflect_code);
						    ...
						    var today=today_date+", verifier "+verifier_name+"-"+verifier_reflect_code;

						    console.log("hello-----------1 ");

						         
						    await Jimp.create(820,520,'#ffffff').then(async nova_new =>{

                console.log("hello-----------2   ");

						         
						        
						                await Jimp.read(`https://ipfs.io/ipfs/${fun_hash}`).then(async newimage => {
                console.log("hello-----------3   ");


						                	            await newimage.resize(720, 520);
						                    ...
						                    text_img.then(result => {
						                        ...
						                        ipfs.files.add(testBuffer,async function (err, file) {
                                                  ...
                                                  console.log("from ipfs self_attested_hash:file[0].hash text_img",file[0].hash);
						                           ...
						                           console.log("jimp_doc ejs",jimp_doc);
						                                      await delay(10000)
             ...
             await web3.eth.getTransactionCount(wallet_address).then( async function(v){
                      ...
                      console.log("*********count*********", count);
                          ...
                          var rawTransaction = {
                              ...
                              "to":contractAddress,                
                              "value": "0x0",
                              "data": contract.methods.addDocument(jimp_doc,verifier_email,client_email,doc_name,verifier_myReflect_code,client_myReflect_code,request_status,reason).encodeABI(),
                              "nonce": web3.utils.toHex(count)
                          }
...
//   const get_digi_hash_verifier = async function(fun_hash,verifier_name,verifier_reflect_code){
