    ...
    var hashes =[];
      ...
      // for(var i=0;i<requestData[0].hase_values.length;i++){
                      let tempForType = content.split("-")
                       var validatore_req_doc_id = content.validatore_req_doc_id
                      ...
                      if (tempForType[0] == "image") {
                        ...
                        new Promise (async (resolve,reject)=>{
                          ...
                          await get_digi_hash(validatore_req_doc_id,srcImage,validatore_name,validatore_req_id,request_id,doc_id,status,request_doc_id ,"image" ,i)
                          .then(responsValue => {
...
//                            6000);
                    ...
                    //      }
                          ...
                          await get_digi_hash(validatore_req_doc_id,tempForType[1],validatore_name,validatore_req_id,request_id,doc_id,status,request_doc_id ,"video",i)
                          .then(responsValue => {
                            if ( responsValue == (requestData[0].hase_values.length-1)) {
                              res.send("done");
                            }
                        ...
                        await get_digi_hash(validatore_req_doc_id,tempForType[1],validatore_name,validatore_req_id,request_id,doc_id,status,request_doc_id ,"pdf",i)
                        .then(responsValue => {
                          if ( responsValue == (requestData[0].hase_values.length-1)) {
                            res.send("done");
                          }
...
const get_digi_hash = async (validatore_req_doc_id,fun_hash,validatore_name,validatore_req_id,request_id,doc_id,status,request_doc_id ,doc_type ,count) => {
  ...
  var today="validatore "+validatore_name;
   ...
   return await new Promise( async (resolve, reject) => {
          ...
          if (doc_type == "image") {
                            ...
                            //  text_date.print(font, x, y + 20,message_date  , 50);
                                                    ...
                                                    text_img.then(result => {
                                                        ...
                                                        ipfs.files.add(testBuffer,async function (err, file) {
                                                          ...
                                                          await  finalsave(validatore_req_doc_id,file[0].hash,doc_type)
                                                          }
            ...
            await  finalsave(validatore_req_doc_id,fun_hash,"video")
        ...
        await  promins.then(async pdf1 =>{
           ...
           const sign_img = await pdfDoc.embedPng(fs.readFileSync(__dirname+'/../../public/assets/images/approve.png'));
            
            let date_time = message_date+" "+message_time
            let ver_text = validatore_name+'-'+validatore_req_id
             ...
             await  ipfs.files.add(testBuffer, async function (err, file) {
               ...
               //  console.log("err from ejs",err);
                 ...
                 await  finalsave(validatore_req_doc_id,file[0].hash,"pdf")
            ...
            await  run(fun_hash);
                ...
                async function finalsave(validatore_req_doc_id,hash,type){


                                

                 await tbl_validatore_requests_doc_files.update({doc_type:type,watermark_hash:hash},{where:{validatore_req_doc_id:validatore_req_doc_id}}).then(async (dataForReturn)=>{
