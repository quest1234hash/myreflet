...
var bufferToDataUrl = require('buffer-to-data-url')
...
exports.cilentInfoViewDetail = async (req,res,nest) => {
  ...
  await db.query('SELECT * FROM `tbl_client_verification_requests` INNER JOIN tbl_wallet_reflectid_rels ON tbl_client_verification_requests.reflect_id=tbl_wallet_reflectid_rels.reflect_id INNER JOIN tbl_user_registrations ON tbl_wallet_reflectid_rels.reg_user_id=tbl_user_registrations.reg_user_id INNER JOIN tbl_manage_category_documents ON ((tbl_manage_category_documents.category_id=tbl_client_verification_requests.sub_category_id)  AND (tbl_manage_category_documents.include="yes")) INNER JOIN tbl_category_documents ON tbl_manage_category_documents.category_doc_id=tbl_category_documents.category_doc_id INNER JOIN tbl_documents_masters ON tbl_documents_masters.doc_id=tbl_category_documents.doc_id WHERE tbl_client_verification_requests.request_id='+request_id+' AND tbl_client_verification_requests.deleted="0"',{type:db.QueryTypes.SELECT})
  .then(async(clientANDrequestData) =>{
    //  console.log("clientANDrequestData.....1213....",clientANDrequestData[0])
    console.log("hellow .......2")   
    await  db.query("SELECT *,GROUP_CONCAT(tbl_request_documents_files.doc_type,'-',tbl_request_documents_files.request_file_hash) as hash FROM tbl_request_documents INNER JOIN tbl_myreflectid_doc_rels ON tbl_request_documents.user_doc_id=tbl_myreflectid_doc_rels.user_doc_id INNER JOIN tbl_documents_masters ON tbl_myreflectid_doc_rels.doc_id=tbl_documents_masters.doc_id INNER JOIN tbl_request_documents_files ON tbl_request_documents_files.request_doc_id=tbl_request_documents.request_doc_id WHERE tbl_request_documents.request_id="+request_id+" AND tbl_request_documents.deleted='0' GROUP by tbl_request_documents_files.request_doc_id",{type:db.QueryTypes.SELECT})
    .then(async(requestUploadDoc) =>{
                 

      const new_requestUploadDoc_array = await requestUploadDoc.map( objData =>{

        const hash_and_type_array  =  objData.hash.split(",")
        const hash_array_with_type = hash_and_type_array.map(hashData =>{
          const temp_splited_data    = hashData.split('-');
          return type_obj = {
            type : temp_splited_data[0],
            hash : temp_splited_data[1]
          }        
        })

        objData["has_array"] = hash_array_with_type

        return objData
      })

      const newes_requestUploadDoc = await  new_requestUploadDoc_array.map(async obj => {
        const new_hash_array = await obj.has_array.map(async hashObj =>{  

          if(hashObj.type != "image") {
            return hashObj;
          }
          else{     
            const url = `https://ipfs.io/ipfs/${hashObj.hash}`
          
            const pdf1 = await  axios.get(url).then(function (response) {
        ...
        const results = await Promise.all(new_hash_array)

        obj.has_array = results
        return obj

      })
      const results2 = await Promise.all(newes_requestUploadDoc)
      console.log("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%",results2[0].has_array)

      res.render('front/sub-verifier/client-info-view-details',{
        session:req.session,
        requestUploadDoc:results2,
      ...
      // var new_array = requestUploadDoc.map((requestUploadDoc)=>{return {doc_type:requestUploadDoc.doc_type,request_file_hash:requestUploadDoc.request_file_hash,request_doc_id:requestUploadDoc.request_doc_id}})

      // var new_array_updated = _.groupBy(new_array,(item)=>{return item.request_doc_id})
      
      // console.log("new_arraynew_arraynew_arraynew_array.....1213....",typeof(new_array_updated))

      // const propertyNames = Object.entries(new_array_updated);
      // console.log("new_arraynew_arraynew_arraynew_array.....1213....",new_array_updated)
    
      //                        console.log("new_arraynew_arraynew_arraynew_array.....1213 pog....",propertyNames.length)
      //                        console.log("new_arraynew_arraynew_arraynew_array.....1213....",new_array_updated.length)
      //                        console.log("hellow .......3")   
                            //  var requestUploadDoc1 =[]

      // if(requestUploadDoc[0]==null){
      ...
      //           async.each(requestUploadDoc,async function (content, cb) {
      //                     // for(var i=0; i<requestUploadDoc.length; i++)
      //                     // {                  
      //                                         console.log("hellow .......4")   
      //                                         // console.log("requestUploadDoc.....1213....",content.request_file_hash)

      //                                         if(content.doc_type == 'image'){

      //                                             //  await  ipfs.files.get(content.request_file_hash, function (err, files) {
      //                                              await  new Promise((resolve ,reject)=>{
      //                                                 request('https://ipfs.io/ipfs/'+content.request_file_hash, function(error, response, body) {
      ...
      //                                                       requestUploadDoc1.push({type:content.doc_type ,doc:content.content})

      //                                                                                         //  res.send(file.content)
      //                                                                                         resolve("done")
      //                                                   //     if(i==(requestUploadDoc.length-1))
      //                                                   //       {
      //                                                   //         //  res.send(requestUploadDoc1)
      //                                                   //               console.log("hellow .......6")   
      //                                                   //                 console.log("hellow .......i ", requestUploadDoc1)
      //                                                   //                res.render('front/sub-verifier/client-info-view-details',{
      //                                                   // session:req.session,
      //                                                   // requestUploadDoc:requestUploadDoc1,
      //                                                   // clientANDrequestData,
      ...
      //                                                       requestUploadDoc1.push({type:content.doc_type ,doc:content.request_file_hash})
      //                                                     //   if(i==(requestUploadDoc.length-1))
      //                                                     //   {
      //                                                     //     //  res.send(requestUploadDoc1)
      //                                                     //           console.log("hellow .......6")   
...
exports.sub_client_list_filter=(req,res,nest)=>{
  ...
  db.query(qre,{type:db.QueryTypes.SELECT})
  .then(clientOfSubVerData =>{
    const clientsArray = paginate(clientOfSubVerData, page, perPage);         
...
exports.submit_add_sub_verfier=(req,res,nest)=>{
  ...
  if(verifier_email!=undefined){
    ...
    MyReflectIdModel.findOne({where:{reflect_code:reflect_code,deleted:"0"}})
    .then(reflectdata=>{
...
exports.delete_sub_verifier=async(req,res,next)=>{
  ...
  for(var i=0; i<invite_sub_id.length; i++){
    await InviteSubVerifier.update(updateObj, { where: { invite_sub_id:invite_sub_id[i] }})
    .then(async(result) =>{
      // console.log(result)
...
exports.delete_indi_sub_veri=async(req,res,next)=>{
  ...
  //  console.log(".............................<><><><.............................................")
  // for(var i=0; i<invite_sub_id.length; i++){
  await InviteSubVerifier.update(updateObj, { where: { invite_sub_id:invite_sub_id }})
  .then(async(result) =>{
      // console.log(result)
      // ClientVerificationModel.findOne({where:{request_id:request_ids[i]}}).then(async(requestdata)=>{
      //   await NotificationModel.create({
...
exports.subverifer_filter=async(req,res,next)=>{
  ...
  db.query(qry,{ type:db.QueryTypes.SELECT})
  .then(sub_verifiers_list=>{
    db.query('SELECT * FROM `tbl_wallet_reflectid_rels` INNER JOIN tbl_user_registrations ON tbl_wallet_reflectid_rels.reg_user_id=tbl_user_registrations.reg_user_id WHERE tbl_wallet_reflectid_rels.user_as="verifier" AND tbl_wallet_reflectid_rels.deleted="0" AND tbl_user_registrations.status="active"',{ type:db.QueryTypes.SELECT})
    .then(verifiersList=>{
      // console.log("sub ver from js",verifiersList)
      const pagedarray = paginate(sub_verifiers_list, page, perPage);  
      console.log(".....................................................................")       
...
exports.sub_verfier_my_client=async(req,res,next)=>{
  ...
  .then( async(sub_verifiers_list)=>{
     
      db.query('SELECT client_request_id FROM `tbl_sub_verifier_clients` where reg_user_id='+userId,{ type:db.QueryTypes.SELECT})
      .then( async(allot_request_id)=>{
        ...
        db.query(query_request,{type:db.QueryTypes.SELECT})
        .then(clientRequsetdata=>{
...
exports.submit_add_client_to_sub=async(req,res,next)=>{
  ...
  for(var i=0; i<sub_verfier_reflect_ids.length;i++){
    ...
    await db.query('SELECT * FROM tbl_wallet_reflectid_rels WHERE tbl_wallet_reflectid_rels.reflect_id='+sub_verfier_reflect_ids[i],{type:db.QueryTypes.SELECT})
    .then(async reflectData=>{ 
      ...
      for(var j=0; j<clients_ids.length;j++){
        ...
        .then(async clientsave=>{
          ...
          await db.query('SELECT * FROM `tbl_client_verification_requests` WHERE `request_id`='+temp,{type:db.QueryTypes.SELECT})
          .then(async requestData=>{ 

            await   UserModel.findOne({where:{reg_user_id:req.session.user_id}})
            .then(async user_data => {
