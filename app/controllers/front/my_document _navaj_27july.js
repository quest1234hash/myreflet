      ...
      for(var i=0;i<allReflect.length;i++)
{
      ...
      for(var j=0;j<all_document_Data.length;j++)
      {
        reflect_code_array.push(all_document_Data[j])
                      ...
                      await  db.query("SELECT DISTINCT reflect_code,user_as,profile_pic,reflect_id,entity_company_name,full_name from  tbl_user_registrations inner join tbl_wallet_reflectid_rels ON tbl_user_registrations.reg_user_id=tbl_wallet_reflectid_rels.reg_user_id  WHERE   tbl_user_registrations.reg_user_id="+user_id,{ type:db.QueryTypes.SELECT}).then(async function(all_reflect_codes){
       ...
       await db.query("SELECT * FROM tbl_wallet_reflectid_rels Where reflectid_by!='digitalWallet' AND reg_user_id="+user_id,{ type:db.QueryTypes.SELECT}).then((myids)=>{
...
exports.add_new_document_request =async (req,res,next )=> {
    ...
    err_msg = req.flash('err_msg');
     ...
     MyReflectIdModel.findOne({where:{reflect_id:reflect_id}}).then(async(result) => {
             ...
             var msg = `${user_data.full_name}- ${result.reflect_code} has request to a create a new document.`
