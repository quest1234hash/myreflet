...
exports.uploadVesionDoc=async(req,res,next)=>{
  ...
  await  form.parse(req, async(err, fields, files) => {
        ...
        var doc_array       = []
        var user_doc_id;
        var exist_doc_array,final_array;

        console.log(".........request_doc_id.......",request_doc_id)
        console.log(".........files.......",files.staff_image)
        console.log(".........type_attr.......",type_attr)


        if(type_attr === 'exist'){
          
                  user_doc_id  = fields.user_doc_id

                   console.log("ifIIIIIIIIIIIIFFFFFFFFFFFFFFFFFFFF")

            await db.query("SELECT * FROM `tbl_myreflectid_doc_rels` INNER JOIN tbl_files_docs on tbl_files_docs.user_doc_id=tbl_myreflectid_doc_rels.user_doc_id WHERE tbl_myreflectid_doc_rels.user_doc_id="+user_doc_id,{ type:db.QueryTypes.SELECT}).then(async function(exit_doc_data){  
                       ...
                       await ClientVerificationModel.findOne({where:{request_id:requestDocData.request_id,deleted:"0"}}).then(async client_v_r_data=>{
                         ...
                         var new_version =parseInt(requestDocData.version_count) +1
					     	     ...
					     	     await RequestDocumentsModel.update({approve_status:"pending",version_count:new_version},{where:{request_doc_id:request_doc_id,deleted:"0"}}).then(async(updata)=>{
                                         ...
                                         await call_exit_file(exit_doc_data,request_doc_id,new_version,exit_doc_data.length,client_v_r_data.client_id,requestDocData.request_id);
                                     })
					         	})
                     })
              ...
              console.log("if")
                       var file_type = files.staff_image.type
  ...
  console.log("files.staff_image.length",coutForSend)
           ...
           var file_type = imagesfromEjs.type
    ...
    await  ipfs.files.add(testBuffer,async function (err, file1) {
      if (err) {
               ...
               }else
               {
                   ...
                   await finalCall(request_doc_id,file1[0].hash,'video',new_version)
         ...
         await  promins.then(async pdf1 =>{
              ...
              await  ipfs.files.add(testBuffer, async function (err, file) {
                ...
                //  console.log("err from ejs",err);
                  ...
                  console.log("from ipfs self_attested_hash:file[0].hash text_img",file[0].hash);
                                      ...
                                      console.log("inner self_att attested :file[0].hash ",file[0].hash);

                                      await finalCall(request_doc_id,file[0].hash,'pdf',new_version)
              ...
              await  run(file1[0].hash);
                                        ...
                                        e = await  ipfs.files.add(testBuffer, async function (err, file) {
                                           ...
                                           console.log("err from ejs",err);
                                               ...
                                               var z;
                                                   ...
                                                   await finalCall(request_doc_id,file[0].hash,'image',new_version)
               ...
               async function finalCall(request_doc_id,hash,type,new_version){
                                       

                                       console.log("request_doc_id ::: ",request_doc_id,'hash ::: ',hash,'  type ::: ',type,'  new_version ::: ',new_version)
                                      await  RequestFilesModel.create({
                                                request_doc_id:request_doc_id,
                                                request_file_hash:hash,
                                                version:new_version,
                                                 doc_type:type
                                               }
...
// await forRender()
     ...
     async function call_exit_file(all_doc,request_doc_id,new_version,exit_doc_length,client_id,request_id){
  ...
  async.each(all_doc,async function (content, cb) {
                  ...
                  if ( content.type == "video" ) {
                     ...
                     await upload_water_mark(content.file_content,"null",user_doc_id,"video",new_version)
                      ...
                      user_doc_id  = content.user_doc_id
                       ...
                       await upload_water_mark(content.self_attested_hash,"yes",user_doc_id,"pdf",new_version)
                                  
                        }
                          else{

                       await upload_water_mark(content.file_content,"no",user_doc_id,"pdf",new_version)
                        }
                     ...
                     if(content.self_attested_hash){
                        ...
                        await request(`https://ipfs.io/ipfs/${content.self_attested_hash}`, async function (error, response, body) {
                                ...
                                if (  !error && response.statusCode == 200  ) {
                                            ...
                                            user_doc_id  = content.user_doc_id
                                            await upload_water_mark(docImage,self_att,user_doc_id,"image",new_version);
                                            i++
                                 } else {
  ...
  async function upload_water_mark(file_hash,self_att,user_doc_id,type,new_version){

    console.log("inside water mark function,,,,,,,,,")


    if ( type == "video" ) {

                     await semiFinalRespone(request_doc_id,file_hash,type,new_version,exit_doc_length)
                ...
                //  console.log("err from ejs",err);
                  ...
                  console.log("from ipfs self_attested_hash:file[0].hash text_img",file[0].hash);
                            ...
                            if(self_att == 'yes'){
                              ...
                              await semiFinalRespone(request_doc_id,OldHash,type,new_version,exit_doc_length)

                            }else{
                                      console.log("inner self_att attested : ",self_att," file[0].hash ",file[0].hash);

                                      await semiFinalRespone(request_doc_id,file[0].hash,type,new_version,exit_doc_length)
                                    ...
                                    var  e = await  ipfs.files.add(testBuffer, async function (err, file) {
                                                    ...
                                                    res.send(err)
                                                                ...
                                                                console.log("hello-----------8 ");
                                                                    ...
                                                                    await semiFinalRespone(request_doc_id,file[0].hash,"image",new_version,exit_doc_length)
...
async function semiFinalRespone(request_doc_id,file_hash,doc_type,new_version,exit_doc_length){


                        console.log("request_doc_id.....",request_doc_id," hash ::: ",file_hash,"  doc_type ::: ",doc_type," version :::: ",new_version," final_valfinal_valfinal_val :::: ",exit_doc_length)

                 await  RequestFilesModel.create({
                                                request_doc_id:request_doc_id,
                                                request_file_hash:file_hash,
                                                version:new_version,
                                                doc_type:doc_type
                                               }).then(async (dataForReturn)=>{

                                                console.log("hello-----------8 ",new_version);
                                                i_val++;
                                              

                                                console.log("exit_doc_length.....",i_val," iiiiiiiiiiiiiiiiiVAAAAAAAAAAAAAALLLLLLLlll.....",exit_doc_length);

                                                  if(exit_doc_length == i_val){
                                                 console.log("exit_doc_length CALLLLLLLLLLLLLLLLLLLLlll.....",exit_doc_length)
