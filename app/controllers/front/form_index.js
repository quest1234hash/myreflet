...
// app.locals.decrypt=crypto.createDecipher('aes256', 'password');
...
exports.test = (req,res, next) => {
  ...
  db.query(' SELECT * FROM `tbl_countries` WHERE status="active" ORDER BY `country_id` ASC',{type:db.QueryTypes.SELECT}).then(countryData=>{
    console.log("fgjdghdhdfhdfh/../////////////")
  })

    console.log("ENNNNNNNNNNNN : ",encrypt('bhavna'))
    console.log("ENNNNNNNNNNNN : ",decrypt('6682011b113168657a27c973ac969f0b'))
  // encrypt('bhavna')
...
exports.submit_register = (req,res,next )=> {
    ...
    console.log("mystrmystrmystr : ",mystr)
  ...
  console.log("--------------------------------------------submitQuestionAns--------------------------------------------")
    ...
    for(var i=0 ; i<question.length; i++){
     ...
     // db.query(sql).then(result=>{
      ...
      console.log("encrypt(answer[i]  : ",encrypt(answer[i]))
   ...
   var otp_expire =now
    ...
    function generateOTP() { 
        ...
        return OTP; 
                 ...
                 console.log("otpotpotpotp resend : ");
         ...
         else
         {
             UserModel.findOne({ where:{email:email,status:'active'} }).then(function(activeUser){
              ...
              else
              {
                ...
                console.log("decrypt(username) : ",decrypt(username))
                console.log("decrypt(verification_code : ",encrypt(verification_code))

                console.log("decrypt(activeEmail) : ",decrypt(activeEmail))
...
exports.resetPinFormPost = (req,res,next )=> {
    var mailid = encrypt(req.body.emailId);
    var pin1 = req.body.otp1.trim();
    var pin2 = req.body.otp2.trim();
    var pin3 = req.body.otp3.trim();
    var pin4 = req.body.otp4.trim();
    var user_pin1 = pin1+""+pin2+""+pin3+""+pin4;  
    var user_pin = decrypt(user_pin1);  
...
exports.test_google_form = (req,res,next)=>{
  res.render('front/test-google-form',{
});
} 