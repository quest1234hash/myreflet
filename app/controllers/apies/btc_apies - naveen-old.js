...
//hash of emailhash
//let h_email=Buffer.from(emailOfHash, 'base64').toString('ascii');
let hashEmail=crypto.createHash('sha256').update(emailOfHash).digest('hex');
emailOfHash=encrypt(hashEmail);
...
//hashing of password
  ...
  let h_password=Buffer.from(req.body.password, 'base64').toString('ascii');
  let hashPass=crypto.createHash('sha512').update(h_password).toString('ascii');
  let enc_pass=encrypt(hashPass);
    ...
    if (userDataResult) {
      ...
      UserModel.findOne({ where: { mobile_number: mobile } }).then(function (userResult) {
        if (userResult) {
          ...
          UserModel.create({ full_name: full_name, last_name: last_name, email: email, country_code_id: country_code_select, mobile_number: mobile, birthplace: place_of_birth, dob: dob, password: enc_pass, otp, otp_expire, complete_steps: steps,hashOfemail:emailOfHash,server_salt:serverSalt }).then(result => {
...
/**Email varification during login**/
exports.varifyEmail=async function(req,res){
    let emailhash=req.body.emailHash;
    console.log(emailhash);
   // let h_email=Buffer.from(emailhash, 'base64').toString('ascii');
    let hashEmail=crypto.createHash('sha256').update(emailhash).digest('hex');
    console.log("hashed:",hashEmail.toString());
     emailHash=encrypt(hashEmail);
      console.log("Enteredddddddddddddddd hash:",emailHash);      
  try{
    UserModel.findOne({ where: { hashOfemail: emailHash } }).then(function (userDataResult) {
      if (userDataResult) {
        console.log("Storedddddddddddddddddddddd hash:",userDataResult.hashOfemail);
        res.json({
          status: 1, msg: "success", data: {
            user_id: userDataResult.reg_user_id,
            server_salt:userDataResult.server_salt
          }
        });
      }else{
        res.json({ status: 0, msg: "failed", data: { err_msg: 'Server salt not found' } });
      }
    }).catch(err => {
      console.log("update faunction err ", err)
      res.json({ status: 0, msg: "failed", data: { err_msg: 'Somthing went wrong try again.' } });
    ...
    if (userDataResult == null) {
      ...
      if (userDataResult.status == "inactive") {
        ...
        let token = await generateAccessToken({ user_id: userDataResult.reg_user_id })
          res.json({ status: 1, msg: "success", data: { user_id: userDataResult.reg_user_id ,token } });
...
exports.submitSetPinAfterLogin = (req, res, next) => {
  ...
  let registration_notification_Token = req.body.registration_notification_Token;
  // var steps=parseInt("4")
  // console.log("1",otp1)
  // console.log("1",otp2)
  // console.log("1",otp3)
 ...
 let email="navee@gmail.com";
  ...
  let hashPin=crypto.createHash('sha512').update(h_pin).toString('ascii');
   console.log("Pin:::::::",hashPin);
   hashPin=encrypt(hashPin);
   console.log("entered pin:::::::::",hashPin);
  //client side work end
  // var ip;
  // if (req.headers['x-forwarded-for']) {
  //     ip = req.headers['x-forwarded-for'].split(",")[0];
      ...
      if (parseInt(result.wrong_otp_count) >= 5) {
        ...
        await UserModel.update({ wrong_otp_count: wrong_otp_count }, { where: { reg_user_id: userid, deleted: "0", status: "active" } })
          .then(async (result) => {
            ...
            res.json({ status: 0, msg: "failed", data: { err_msg: 'You entered wrong PIN.', user_id: userID } });
        ...
        .then(async (result) => {
          ...
          await tbl_log_manage.create({
            ...
            if (user_token_exist != null) {
              ...
              res.json({ status: 1, msg: "success", data: { user_id: userID,token } });
