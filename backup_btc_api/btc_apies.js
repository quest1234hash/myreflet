...
//push notification
const pushnotification = async (userId, titles, bodys) => {
...
exports.getNotifications=async function(req,res){
  ...
  try{
        let notificationDet= await NotificationModel.findAll({where:{receiver_id:user_id}, offset: page_num,limit:10,order: sequelize.literal('notification_id DESC')});
        let notificationArr=[];
...
exports.getAllDigitalWallets=async function(req,res){
  let user_id=req.body.user_id;
  try{
           let digitals=await DigitalWalletRelsModel.findAll({where:{reg_user_id:user_id,status:'active'}});
           let digitalArr=[];
...
const updateNotification=async function(sender_userid,receiver_userid,msg,subject,profile_pic){
            ...
            console.log("Walllllllll ",wallet_address);
                 ...
                 let refDoc=await DocumentReflectIdModel.findOne({where:{user_doc_id:file_det.user_doc_id}});
                    ...
                    formatted = dt.format('m/d/Y');
                     ...
                     operation:docs_history[i].operation,
                     status:'success',
                     wallet_type:wallet_type
                   }
                   transArr[i]=transObj;
    ...
    if(wallet_type.toLowerCase()=='btc'||wallet_type.toLowerCase()=='ethereum'){
      ...
      //   let sender_info_wallet=await CryptoWalletModel({where:{wallet_address:transDet.}})
        ...
        if(transDet.action=='shared'){
          rec_name=decrypt1(transDet.receiver_name);
   ...
   var sender_reflet_id=senderInfo.reflect_code;
    ...
    //var privKeyWIF = wif.encode(128,privateKey,true);
        let balanceInBTC= await btcbalance(sender_wallet_address);
  		...
  		// console.log("balanc",balanceInBTC);
	...
	// console.log("Hash: ",res.body.tx.hash,"fees",res.body.tx.fees);
     ...
     async function callback(error, response, body) {
          ...
          if (!error) {
            ...
            //  console.log("fees",result_data.tx.fees);
            //  console.log("hash",result_data.tx.hash);
              //console.log("hash",result_data.tx.hash);
...
// var senderAmountInDollar=parseFloat(amount)*parseFloat(currentPriceInUsd.price);
// senderAmountInDollar= senderAmountInDollar.toString();
// let receiver_get_amount=parseFloat(amount)-parseFloat(fees_btc);
// var receiverAmountInDollar=receiver_get_amount*parseFloat(currentPriceInUsd.price);
// receiverAmountInDollar=receiverAmountInDollar.toString();
...
//     sender_wallet_id:encrypt1(sender_wallet_address),
//     sender_reg_user_id:sender_user_id,
//     transaction_hash:result_data.tx.hash,
//     sender_reflet_id:sender_reflet_id,
//     receiver_wallet_id:encrypt1(receiver_wallet_address),
//     receiver_reg_user_id:receiver_user_id,
//     receiver_reflect_id:receiver_reflet_id,
...
//           //receiver notification
//           if(receiver_user_id!=''){
//             var msg3=`You have received amount ${receiver_get_amount} BTC from ${sender_name}.`
...
//           }
          ...
          //   let trans_msg={success:1,msg:"Transaction done successfully.",txHash:result_data.tx.hash,fees:fees_btc};
            // res.json({status:1,msg:'Transaction done successfully',data:{txHash:result_data.tx.hash,fees:fees_btc}});
